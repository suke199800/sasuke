...
def ask_gemini():
    ...
    try:
        ...
        answer_text = ""
        if hasattr(response, 'parts') and response.parts: answer_text = "".join(part.text for part in response.parts)
        elif hasattr(response, 'text'): answer_text = response.text
        if not answer_text:
            if hasattr(response, 'prompt_feedback') and response.prompt_feedback.block_reason: reason = response.prompt_feedback.block_reason; answer_text = f"송구하오나, 답변드릴 수 없사옵니다. (사유: {reason})"
            else: answer_text = "송구하오나, 답변을 마련하는 중에 문제가 생겼사옵니다."
...
def add_guestbook_entry():
    data = request.get_json()
    name = data.get('name', '').strip(); message = data.get('message', '').strip()
    if not name or not message: return jsonify({"error": "성함과 남기실 말씀을 모두 적어주시옵소서."}), 400
    if len(name) > 50 or len(message) > 1000: return jsonify({"error": "글자 수 제한을 확인해주시옵소서."}), 400
    ...
    if conn:
        try:
            with conn.cursor() as cur:
                cur.execute("INSERT INTO guestbook (name, message) VALUES (%s, %s) RETURNING name, message, timestamp", (name, message))
                inserted_row = cur.fetchone(); conn.commit()
                if inserted_row:
                    formatted_timestamp = inserted_row[2].astimezone(TIMEZONE).strftime('%Y-%m-%d %H:%M:%S')
                    new_entry_data = {"name": inserted_row[0], "message": inserted_row[1], "timestamp": formatted_timestamp}
                    app.logger.info(f"새 방명록 등록: {new_entry_data}")
                    socketio.emit('new_entry', new_entry_data)
        except Exception as e:
            conn.rollback(); app.logger.error(f"DB에 방명록 저장 중 오류: {e}")
            return jsonify({"error": "방명록 저장 중 서버 오류 발생."}), 500
        finally: conn.close()
        if new_entry_data: return jsonify({"success": True, "entry": new_entry_data}), 201
        else: return jsonify({"error": "방명록 저장 확인 중 문제 발생."}), 500
    else: return jsonify({"error": "데이터베이스 연결 실패."}), 500
